...
class UpdateAbl {
  ...
  async update(awid, dtoIn, uuAppErrorMap = {}) {
    ...
    location = await this.locationDao.get(awid, id);

    // 3.1
    if (!location) {
      throw new Errors.Update.LocationDoesNotExist({ uuAppErrorMap }, { id: id });
    }

    // HDS 4

    try {
      location = await this.locationDao.update({ awid, ...dtoIn });
    } catch (e) {
      // 4.1
